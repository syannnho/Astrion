...
local Window = WindUI:CreateWindow({
    Title = "AstrionHUB | MOUNT PEDAUNAN",
    Author = "by Jinho",
    Folder = "AstrionHUB_MountPedaunan",
...
local jsonFolder = mainFolder .. "/js_mount_arunika_patch_001"
...
local baseURL = "https://raw.githubusercontent.com/RullzsyHUB/roblox-scripts-json/refs/heads/main/json_mount_pedaunan/"
local jsonFiles = {
    "spawnpoint.json",
    "checkpoint_1.json",
    "checkpoint_2.json",
...
AutoWalkTab:Toggle({
    ...
    Desc = "Tampilkan menu pause dan rotate",
    Default = false,
    Callback = function(Value)
        if Value then
...
AutoWalkTab:Toggle({
    ...
    Desc = "Aktifkan looping otomatis",
    Default = false,
    Callback = function(Value)
...
local function createStatusLabel()
    updateStatusLabel = UpdateTab:Section({
...
CreditsTab:Button({
    ...
    Justify = "Center",
    Callback = function()
        local discordLink = "https://discord.gg/yQpag5BmH"
        if setclipboard then
            setclipboard(discordLink)
            WindUI:Notify({
                Title = "Discord",
                Content = "Link Discord disalin!",
                Icon = "clipboard"
            })
        end
    end
